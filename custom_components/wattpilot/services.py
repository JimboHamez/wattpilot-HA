...
async def async_service_SetNextTrip(hass: HomeAssistant, call: ServiceCall) -> None:
    ...
    try:
        ...
        if charger is None:
            _LOGGER.error("%s - async_service_SetNextTrip: unable to get charger for: %s", DOMAIN, CONF_DEVICE_ID)
            return None
    ...
    try:
        ...
        if api_state is True:
            ...
            if not timeout > timer:
                # nosemgrep: python.lang.security.audit.logging.logger-credential-leak.python-logger-credential-disclosure -- logs only the timeout duration, never the key  # noqa: E501
                _LOGGER.error(
                    "%s - async_service_SetGoECloud: Timeout - api key not available after: %s sec", DOMAIN, timeout
                )
...
async def async_service_SetDebugProperties(hass: HomeAssistant, call: ServiceCall) -> None:
    ...
    try:
        ...
        if entry_data is None:
            _LOGGER.error(
                "%s - async_service_SetDebugProperties: unable to get entry data for: %s", DOMAIN, CONF_DEVICE_ID
            )
            return None
...
async def async_service_ReConnectCharger(hass: HomeAssistant, call: ServiceCall) -> bool:
    ...
    try:
        ...
        if entry_data is None:
            _LOGGER.error(
                "%s - async_service_ReConnectCharger: unable to get entry data for: %s", DOMAIN, CONF_DEVICE_ID
            )
            return False
