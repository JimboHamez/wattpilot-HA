...
class ChargerSelect(ChargerPlatformEntity, SelectEntity):
    ...
    async def _async_update_validate_platform_state(self, state=None):
        ...
        try:
            if state in self._opt_dict:
                ...
                _LOGGER.error("%s - %s: _async_update_validate_platform_state failed: state %s not within options: %s", self._charger_id, self._identifier, state, self._opt_out)
                return None
            return state
        except Exception as e:
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        try:
            ...
            if key is None:
                _LOGGER.error("%s - %s: async_select_option: option %s not within options: %s", self._charger_id, self._identifier, option, self._opt_out)
                return None
